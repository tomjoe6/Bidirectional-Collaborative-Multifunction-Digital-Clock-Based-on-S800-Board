...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # Timer for auto day/night based on sunrise/sunset
        self._daynight_timer = QTimer(self)
        self._daynight_timer.setInterval(60000)
        self._daynight_timer.timeout.connect(self._on_daynight_check)
        self._last_daynight_mode: str = ""
    ...
    def _create_toolbar(self) -> QHBoxLayout:
        ...
        # Night mode indicator (moved from twin panel to toolbar)
        self.lbl_night = QLabel("DAY")
        self.lbl_night.setStyleSheet("color: #FFAA00; font-weight: bold;")
        layout.addWidget(self.lbl_night)

        # Mic button for voice commands
        self.btn_mic = QPushButton("🎤")
        self.btn_mic.setFixedWidth(40)
        self.btn_mic.setToolTip("语音指令：设X点Y分闹钟 / 天气怎么样")
    ...
    def _create_twin_panel(self) -> QWidget:
        ...
        # 7-segment display (centered)
        self.seven_seg = SevenSegWidget()
        grp_layout.addWidget(self.seven_seg, alignment=Qt.AlignCenter)
    ...
    def _create_log_panel(self) -> QWidget:
        """Create the bottom area with log + dashboard tabs."""
        from PyQt5.QtWidgets import QTabWidget
        from dashboard import Dashboard, LogStore
        ...
        self.log_panel = LogPanel(store=self._log_store)
        log_layout.addWidget(self.log_panel)
        tabs.addTab(log_page, "通信日志")

        # Tab 2: Dashboard
        self._dashboard = Dashboard(store=self._log_store)
        tabs.addTab(self._dashboard, "数据看板")
    ...
    def _on_worker_connected(self, success: bool, message: str) -> None:
        ...
        if success:
            ...
            self._last_daynight_mode = ""
            self.log_panel.addEntry("OK", f"串口 {message} 连接成功")
            # Run first check immediately
            self._on_daynight_check()
            # Auto-fetch weather on connect for sunrise/sunset
            if self._weather_client.has_api_key:
                QTimer.singleShot(3000, self._on_weather_fetch)

            # Holiday greeting after boot
    ...
    def _on_send_command(self, cmd: str) -> None:
        ...
        # Log TX — show grouped (names first, values after) for readability
        stripped = cmd.strip()
        display = ProtocolParser.format_display_from_raw(stripped) if stripped.upper().startswith("*SET") else stripped
        self.log_panel.addEntry("TX", display)

        # Track format changes locally
        upper = stripped.upper()
        if "*SET:FORMAT RIGHT" in upper or "*SET:FORMAT RIGHT" == upper[:19]:
            self._twin_state._format = "RIGHT"
            self._twin_state.format_changed.emit("RIGHT")
            self._update_status_text()
        elif "*SET:FORMAT LEFT" in upper or "*SET:FORMAT LEFT" == upper[:18]:
            self._twin_state._format = "LEFT"
            self._twin_state.format_changed.emit("LEFT")
            self._update_status_text()
    ...
    def _on_ntp_sync(self) -> None:
        ...
        self.log_panel.addEntry("TX", "NTP对时: DATE+TIME")
        self._on_send_command(date_cmd)
        self._on_send_command(time_cmd)

        # Light D6 after 1.5s, then turn off after 5s
        QTimer.singleShot(1500, self._on_ntp_done)

    def _on_ntp_done(self) -> None:
        """NTP complete — light D6, restore status."""
        self.log_panel.addEntry("OK", "对时完成 ✓")
    ...
    def _toggle_voice(self) -> None:
        ...
        try:
            ...
            self.btn_mic.setText("🔴")
            self.log_panel.addEntry("OK", "语音: 正在监听...")
        except Exception as e:
            self._voice_worker = None
            self.btn_mic.setText("🎤")
            self.log_panel.addError(f"语音启动失败: {e}")

    def _on_voice_finished(self) -> None:
        """Reset voice UI when the worker exits after an init/runtime error."""
        self._voice_worker = None
        self.btn_mic.setText("🎤")

    def _on_voice_alarm(self, hour: int, minute: int) -> None:
        """Handle voice alarm command."""
        ...
        try:
            ...
            self.log_panel.addError(f"语音闹钟失败: {e}")
    ...
    def _on_daynight_check(self) -> None:
        """Check time and auto-switch DAY/NIGHT mode.
        Uses sunrise/sunset from weather API when available,
        falls back to fixed 6:00/18:00."""
        if not self._serial_worker or not self._serial_worker.is_connected():
            return

        now = time.localtime()
        cache = self._weather_client.cache
        use_sun = (cache.result is not None and cache.result.success
                   and cache.result.sunrise > 0 and cache.result.sunset > 0)

        if use_sun:
            now_ts = time.time()
            sr = cache.result.sunrise
            ss = cache.result.sunset
            new_mode = "NIGHT" if (now_ts < sr or now_ts > ss) else "DAY"
        else:
            new_mode = "NIGHT" if (now.tm_hour >= 18 or now.tm_hour < 6) else "DAY"

        if new_mode != self._last_daynight_mode:
            self._last_daynight_mode = new_mode
            cmd = f"*SET:MODE {new_mode}\r\n"
            hint = ""
            if use_sun:
                hint = f"(日出{time.ctime(int(sr))[11:16]} 日落{time.ctime(int(ss))[11:16]})"
            else:
                hint = "(兜底: 18:00-06:00)"
            self.log_panel.addEntry("TX", f"自动{new_mode}{hint}")
            try:
                self._serial_worker.send(cmd.encode("ascii"))
            except Exception:
                pass
        else:
            self.log_panel.addEntry("EVT", f"昼夜检查: 保持{new_mode}")
    ...
    def _on_latency_updated(self, latency_ms: int) -> None:
        ...
        self._log_store.add_latency(latency_ms)
        if latency_ms < 50:
    ...
    def _on_mode_changed(self, mode: str) -> None:
        ...
        self.lbl_night.setText(mode.upper())
        if night:
            self.lbl_night.setStyleSheet("color: #4444FF; font-weight: bold;")
        else:
            self.lbl_night.setStyleSheet("color: #FFAA00; font-weight: bold;")
    ...
    def _on_edit_event(self, edit_type: str, edit_value: str) -> None:
        """Update twin panel to mirror board edit display."""
        # edit_value is a formatted string like "240101" (DATE) or "120000" (TIME)
        text = edit_value.ljust(8)[:8]
        self.seven_seg.setText(text)
