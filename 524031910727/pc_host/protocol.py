...
class ProtocolParser:
    ...
    # Buffer for partial lines across read() calls
    _partial: str = ""
    ...
    def parse_incoming(data: bytes) -> List[Dict[str, Any]]:
        ...
        text = ProtocolParser._partial + text
        ProtocolParser._partial = ""

        # If the last character is not a line ending, buffer it for next time
        if text and text[-1] not in ('\r', '\n'):
            last_cr = text.rfind('\r')
            last_lf = text.rfind('\n')
            last_nl = max(last_cr, last_lf)
            if last_nl >= 0:
                ProtocolParser._partial = text[last_nl + 1:]
                text = text[:last_nl + 1]
            else:
                ProtocolParser._partial = text
                return []
    ...
    def format_command(
        ...
        """Format a command string (interleaved name/value)."""
        if subcmd:
    ...
    @staticmethod
    def format_display(cmd: str, subcmd: Optional[str] = None,
                       params: Optional[List[str]] = None) -> str:
        """Same as format_command but shows names-first in the log."""
        raw = ProtocolParser.format_command(cmd, subcmd, params)
        if not params:
            return raw
        names, values = [], []
        for i, p in enumerate(params):
            if i % 2 == 0:
                ...
                values.append(str(p))
        if len(names) == len(values) and all(
            not v[0].isdigit() and v[0] not in ('-', '+') for v in names
        ):
            parts = raw.split(" ", 1)
            if len(parts) == 2:
                return f"{parts[0]} {' '.join(names)} {' '.join(values)}"
        return raw
    ...
    @staticmethod
    def format_display_from_raw(raw: str) -> str:
        """Convert interleaved TX to grouped display format.
        e.g. *SET:TIME HOUR 12 MIN 30 → *SET:TIME HOUR MIN 12 30"""
        parts = raw.split(" ", 1)
        if len(parts) != 2:
            return raw
        ...
        for i, t in enumerate(tokens):
            if i % 2 == 0:
                ...
                values.append(t)
        if len(names) == len(values) and all(
            not v[0].isdigit() for v in names if v
        ):
            return f"{parts[0]} {' '.join(names)} {' '.join(values)}"
        return raw
