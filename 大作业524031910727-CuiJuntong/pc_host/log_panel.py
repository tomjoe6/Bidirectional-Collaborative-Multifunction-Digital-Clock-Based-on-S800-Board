...
class LogPanel(QWidget):
    ...
    def __init__(self, parent: QWidget = None, store=None) -> None:
        """Initialize the log panel.

        Args:
            parent: Optional parent widget.
            store: Optional LogStore for SQLite persistence.
        """
    ...
    def addEntry(self, direction: str, content: str,
        ...
        # Persist to SQLite (skip noisy heartbeat events)
        if self._store is not None:
            from dashboard import _skip_heartbeat, classify
            if not _skip_heartbeat(direction, content):
                cat = classify(direction, content)
                if cat is not None:
                    self._store.add(cat, content)
