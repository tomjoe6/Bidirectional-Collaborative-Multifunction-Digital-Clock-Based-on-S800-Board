    ...
    while (1) {
        ...
        if (g_flag_10ms) {
            ...
            if (boot_done) {
                ...
                if (g_msg_timeout > 0 && Keys_GetEditState() == EDIT_NONE) {
                    g_msg_timeout--;
                    if (g_msg_timeout == 0 && g_disp_mode == DISP_MODE_FULL) {
                        g_disp_mode = DISP_MODE_TIME;
                        Display_UpdateFromClock(&g_clock);
                    }
                }
...
void Display_FillFromBuffer(void)
{
    ...
    /* FORMAT_RIGHT reverses static display content. For scrolling content,
     * FORMAT controls the flow direction instead, so do not reverse the
     * visible window and cancel the user's selected direction. */
    if (g_disp_format == FORMAT_RIGHT &&
        !((g_disp_mode == DISP_MODE_FULL || g_disp_mode == DISP_MODE_YEAR)
          && str_len > DISP_DIGITS)) {
